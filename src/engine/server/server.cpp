...
void CServer::ProcessClientPacket(CNetChunk *pPacket)
{
	...
	if(Sys)
	{
		...
		else if(Msg == NETMSG_REQUEST_MAP_DATA)
		{
			...
			if (Chunk != m_aClients[ClientID].m_NextMapChunk)
			{
			...
			SendMapData(ClientID, 15 + m_aClients[ClientID].m_NextMapChunk);
...
void CServer::UpdateRegisterServerInfo()
{
	...
	sha256_str(m_pMap->Sha256(), aMapSha256, sizeof(aMapSha256));
...
void CServer::DemoRecorder_HandleAutoStart()
{
	if (g_Config.m_SvAutoDemoRecord)
	{
		...
		m_DemoRecorder.Start(Storage(), m_pConsole, aFilename, GameServer()->NetVersion(), (char*)m_pCurrentMapData, m_CurrentMapCrc, "server");
